...
# Determine device: prefer MPS (Mac GPU) if available, else CPU
...
# XTTS has issues with MPS (ComplexFloat not supported), so force CPU
xtts_tts = TTS(XTTS_MODEL).to("cpu")
...
# ---- Static File Serving for Production (Docker/HF Spaces) ----
# In production, we copy the React build (app/dist) to a folder named 'static'
...
if STATIC_DIR.exists():
    ...
    # Catch-all for client-side routing (e.g. /profile, /settings)
    # WARNING: API routes must be defined BEFORE this matches
    @app.get("/{full_path:path}")
    def serve_react_app(full_path: str):
        # If it matches a file, serve it
    ...
    print("Static directory not found. Running in API-only mode (Local Dev).")
    @app.get("/")
    def home():
        return {"status": "running", "message": "Local Voice TTS Backend is ready! (Frontend not served)"}


if __name__ == "__main__":
    import uvicorn
    # VERO_ENV can be set to use port 7860 default for HF Spaces
